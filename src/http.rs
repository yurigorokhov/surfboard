...
pub struct Client {
    ...
    // read_record_buffer: [u8; TLS_DATA_BUFFER_SIZE],

    // Buffer for transmitted TLS data
    // write_record_buffer: [u8; TLS_DATA_BUFFER_SIZE],
}

impl Client {
    /// Create a new client
    pub fn new(stack: Stack<'static>, _rng: RngWrapper) -> Self {
        ...
        Self {
            ...
            // rng,
            tcp_client_state,
            // read_record_buffer: [0_u8; TLS_DATA_BUFFER_SIZE],
            // write_record_buffer: [0_u8; TLS_DATA_BUFFER_SIZE],
        }
    }
}
...
impl ClientTrait for Client {
    async fn send_request(&mut self, url: &str) -> Result<Vec<u8, RESPONSE_SIZE>, Error> {
        ...
        debug!("Response status: {:?}", response.status.0);
